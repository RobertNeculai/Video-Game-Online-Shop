window.checkout = {
    ...
    getCustomerID: function () {
        ...
        $.ajax({
            url: checkout.API_URL + "/customers/userid/" + userId,
            method: "GET",
            contentType: "application/json"
        }).done(function (response) {
            checkout.getCustomer(response.id)

        })
    },
    getCustomer: function (customerId) {
        $.ajax({
            url: checkout.API_URL + "/customers/" + customerId,
            method: "GET",
            contentType: "application/json"
        }).done(function (response) {
            $('#billing_first_name_field').html(checkout.displayCheckoutforCustomer(response));
        })
    },
    displayCheckoutforCustomer: function (customer) {
        return `
                                                <label class="" for="billing_first_name">First Name <abbr title="required" class="required">*</abbr>
                                                </label>
                                                <input type="text" value="${customer.firstName}" placeholder="" id="billing_first_name" name="billing_first_name" class="input-text ">

                                            <p id="billing_last_name_field" class="form-row form-row-last validate-required">
                                                <label class="" for="billing_last_name">Last Name <abbr title="required" class="required">*</abbr>
                                                </label>
                                                <input type="text" value="${customer.lastName}" placeholder="" id="billing_last_name" name="billing_last_name" class="input-text ">
                                            </p>
                                            <div class="clear"></div>
                                            ...
                                            <p id="billing_address_1_field" class="form-row form-row-wide address-field validate-required">
                                                ...
                                                <input type="text" value="${customer.address}" placeholder="Street address" id="billing_address_1" name="billing_address_1" class="input-text ">
                                            </p>
                                            ...
                                            <p id="billing_email_field" class="form-row form-row-first validate-required validate-email">
                                                ...
                                                <input type="text" value="${customer.email}" placeholder="" id="billing_email" name="billing_email" class="input-text ">
                                            </p>
